...
app.get('/couriers', async (req, res) => {
    try {
        ...
        const tasks = dbTasks.map(({id, text, mass, district, adress }) => ({taskID: id, text, mass, district, adress}))
        
        const couriers = dbCouriers.map(courier => ({ courierID: courier.id, name: courier.name, district: courier.district, tasks: tasks.filter(task => courier.tasks.indexOf(task.taskID) !== -1)}))
        ...
        res.json({
            ...
            statusCode: 500,
            message: `Getting couriers and tasks error: ${err.error.message || err.error}`
        })
    }
})
...
app.post('/couriers', async (req, res) => {
    try {
        const {courierID, name, district} = req.body
        await db.addCourier({courierID, name, district})
        res.statusCode = 200
        res.statusMessage = 'OK'
        res.send()
    } catch(err) {
...
app.post('/tasks', async (req, res) => {
    try {
        const {taskID, text, mass, district, adress, courierID} = req.body
        await db.addTask({taskID, text, mass, district, adress, courierID})
        res.statusCode = 200
        res.statusMessage = 'OK'
        res.send()
    } catch(err) {
...
app.patch('/couriers/:courierID', async (req, res) => {
    try {
        ...
        await db.updateCourier({courierID, name, district})
        res.statusCode = 200
        res.statusMessage = 'OK'
        res.send()
    } catch(err) {
